...
export default function DegradasiTable() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/degradasi/${id}`, { method: "DELETE" });
      if (res.ok) {
          ...
          <Table>
            ...
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                ...
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Jenis Kawasan</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Jenis Tutupan Lahan</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Biomassa / Kerapatan Tajuk</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Kondisi Vegetasi</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Gangguan</TableCell>
              ...
              {loading ? (
                ...
                data.map((row: any, idx: number) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.jenis_kawasan}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.jenis_tutupan_lahan}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.biomassa_kerapatan_tajuk}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.kondisi_vegetasi}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.gangguan}</TableCell>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                      <Link href={`/degradasi/${row.id}/edit`}>
                        <button 
