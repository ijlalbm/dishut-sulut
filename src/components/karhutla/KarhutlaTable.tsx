...
export default function KarhutlaTable() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/karhutla/${id}`, { method: "DELETE" });
      if (res.ok) {
          ...
          <Table>
            ...
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                ...
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Kawasan hutan lindung, hutan produksi, apl, lainnya</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Akses Lokasi Mudah, Sedang, Sulit</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Jenis Lahan Terbakar</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Luas Lahan Terbakar (Ha)</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Tingkat Keparahan</TableCell>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Sumber Kebakaran</TableCell>
              ...
              {loading ? (
                ...
                data.map((row: any, idx: number) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.akses_lokasi}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.jenis_lahan_terbakar}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.luas_lahan_terbakar}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.tingkat_keparahan}</TableCell>
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">{row.sumber_kebakaran}</TableCell>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                      <Link href={`/karhutla/${row.id}/edit`}>
                        <button 
