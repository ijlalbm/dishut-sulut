...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === "GET") {
      const [rows] = await pool.query("SELECT * FROM karhutla WHERE id = ?", [id]);
    ...
    if (req.method === "PUT") {
      ...
      const newAksesLokasi = akses_lokasi ?? existing.akses_lokasi;
      const newJenisLahanTerbakar = jenis_lahan_terbakar ?? existing.jenis_lahan_terbakar;
      const newLuasLahanTerbakar = luas_lahan_terbakar ?? existing.luas_lahan_terbakar;
      const newTingkatKeparahan = tingkat_keparahan ?? existing.tingkat_keparahan;
      const newSumberKebakaran = sumber_kebakaran ?? existing.sumber_kebakaran;
      ...
      await pool.query(
        ...
        [
          ...
          newKawasan,
          newAksesLokasi,
          newJenisLahanTerbakar,
          newLuasLahanTerbakar,
          newTingkatKeparahan,
          newSumberKebakaran,
          newPenyuluhId,
          id,
    ...
    if (req.method === "DELETE") {
      const [result] = await pool.query("DELETE FROM karhutla WHERE id = ?", [id]);
